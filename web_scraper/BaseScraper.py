...
UrlProcessFunction = Callable[[list[Url | str]], list[Url]]
IdGeneratorFunction = Callable[[Url], str | None]
...
class BaseScrapper:
    ...
    def __init__(
        ...
        web_base_url: str,
        web_pag_url: str,
        urls_filter_function: UrlProcessFunction,
        id_generator_function: IdGeneratorFunction,
    ...
    async def scrape_pagination(self, content: DocContent) -> list[Url] | None:
        ...
        offers: list[Url] = self.urls_filter_funcition(a_elements)
    ...
    async def generate_page_path(self, url: Url) -> Path | None:
        ...
        """
        id = self.id_generator_function(url)
        if id is None:
