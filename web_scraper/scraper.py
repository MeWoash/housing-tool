...
async def create_query_url(page_number: int = 1) -> Url:
    return Url(BASE_URL + f"/pl/wyniki/sprzedaz/mieszkanie/zachodniopomorskie/szczecin/szczecin/szczecin?limit=72&ownerTypeSingleSelect=ALL&by=LATEST&direction=DESC&viewType=listing&page={page_number}")
...
async def get_offers_from_page(page_number: int) -> list[Url]:
    query_url = await create_query_url(page_number)

    content = await download_page(query_url)

    selector = Selector(content)
    a_elements = selector.xpath("//a/@href").getall()
    offers: list[Url] = [Url(BASE_URL + offer) for offer in a_elements if offer.startswith("/pl/oferta")]
